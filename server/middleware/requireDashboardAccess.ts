...
export function requireDashboardAccess(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  if (!hasSession) return res.status(401).json({ error: 'Login required' });
  ...
  if (!emailLc) return res.status(403).json({ error: 'Google account not connected' });
  if (!ALLOWED_GOOGLE_EMAILS.includes(emailLc)) return res.status(403).json({ error: 'Google account not allowed' });
